...
Table~\ref{tab:results} presents test set accuracy results for VANS across all seven I-RAVEN configurations. The model achieves an overall accuracy of 86.2\%, meeting our target objective of 85\%+. Performance varies across configurations, with simpler structures (center\_single, left\_right) achieving higher accuracy (90\%+) while more complex nested structures (in\_out variants) show lower but still competitive performance (78-82\%).
...
\midrule
center\_single & 91.3 & 2000 \\
distribute\_four & 88.7 & 2000 \\
distribute\_nine & 85.2 & 2000 \\
in\_out\_center & 81.9 & 2000 \\
in\_out\_grid & 78.4 & 2000 \\
left\_right & 90.1 & 2000 \\
up\_down & 89.6 & 2000 \\
\midrule
\textbf{Overall} & \textbf{86.2} & \textbf{14000} \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
...
Table~\ref{tab:baselines} compares VANS against prior methods on the RAVEN dataset. While direct comparison is limited by dataset differences (RAVEN vs. I-RAVEN), our results demonstrate significant improvements over early approaches. VANS achieves performance comparable to or exceeding ResNet+DRT (59.56\% on RAVEN), while operating on the more challenging I-RAVEN dataset with bias corrections.
...
We analyze model behavior through error patterns and attention visualizations. The model shows strongest performance on problems with clear row/column progressions (left\_right, up\_down), where structural embeddings effectively capture the pattern. Nested configurations (in\_out variants) present greater challenges, likely due to increased complexity in spatial relationships. Common failure modes include composite transformations, where problems require multiple simultaneous attribute changes that challenge the model's ability to capture all relevant pattern aspects simultaneously. Another frequent failure mode involves subtle pattern variations, where correct and incorrect candidates differ minimally, making discrimination difficult even when the model has correctly identified the underlying pattern. Additionally, complex nesting with deeply nested structures containing multiple hierarchical levels presents particular challenges, as the model must reason across multiple levels of abstraction simultaneously.
...
We presented VANS, a transformer-based architecture for solving Raven's Progressive Matrices through context-based reasoning. Our approach combines frozen DINOv2-L visual features with a lightweight reasoning module, achieving 86.2\% accuracy on the I-RAVEN test set, meeting our target objective of 85\%+.

\subsection{Objectives Assessment}

All three project objectives were successfully met. For architecture design, we designed and implemented a transformer-based architecture that effectively encodes context and reasons about candidates, achieving 86.2\% accuracy, which exceeds our 85\% target. For feature representation, we demonstrated that frozen DINOv2 features provide effective visual representations for RPM solving, enabling strong performance without task-specific visual training, confirming the transferability of foundation model features to visual reasoning tasks. For performance evaluation, we systematically evaluated performance across all seven I-RAVEN configurations, identifying strengths such as strong performance on simple progressions and limitations such as reduced accuracy on complex nesting structures.
...
In summary, VANS demonstrates that context-based reasoning with foundation model features can effectively solve Raven's Progressive Matrices, providing a strong baseline for future work in visual analogical reasoning.
