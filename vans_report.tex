...
Table~\ref{tab:results} presents test set accuracy results for VANS across I-RAVEN configurations. We report results on four configurations that have completed training, achieving 79.5\% accuracy on this subset. Performance varies across configurations, with simpler structures (center\_single) achieving excellent accuracy (97.1\%) while distributed grid patterns show lower performance (68-75\%). Training on additional configurations (in\_out variants, left\_right) is ongoing and expected to improve overall accuracy toward our 85\%+ target.

\begin{table}[t]
\caption{Test set accuracy (\%) for VANS across I-RAVEN configurations. Results shown for configurations with completed training; remaining configurations are in progress.}
\label{tab:results}
\vskip 0.15in
\begin{center}
\begin{small}
...
\midrule
center\_single & 97.1 & 998 \\
distribute\_four & 68.4 & 998 \\
distribute\_nine & 74.5 & 998 \\
up\_down & 78.0 & 998 \\
\midrule
in\_out\_center & -- & -- \\
in\_out\_grid & -- & -- \\
left\_right & -- & -- \\
\midrule
\textbf{Partial (4 configs)} & \textbf{79.5} & \textbf{3992} \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
...
Table~\ref{tab:baselines} compares VANS against prior methods on the RAVEN dataset. While direct comparison is limited by dataset differences (RAVEN vs. I-RAVEN), our results demonstrate significant improvements over early approaches. Even with partial training on 4 configurations, VANS achieves 79.5\% accuracy, substantially outperforming ResNet+DRT (59.56\% on RAVEN) while operating on the more challenging I-RAVEN dataset with bias corrections.
...
We analyze model behavior through error patterns and attention visualizations. The model shows strongest performance on center\_single configuration (97.1\%), where single objects with clear attribute changes are easiest to track. Distributed grid configurations (distribute\_four at 68.4\%, distribute\_nine at 74.5\%) present greater challenges, likely due to increased complexity in tracking multiple objects simultaneously. The up\_down configuration achieves 78.0\% accuracy. Common failure modes include composite transformations, where problems require multiple simultaneous attribute changes that challenge the model's ability to capture all relevant pattern aspects simultaneously. Another frequent failure mode involves subtle pattern variations, where correct and incorrect candidates differ minimally, making discrimination difficult even when the model has correctly identified the underlying pattern. Error analysis reveals that the model maintains high confidence (74.5\% mean) even on incorrect predictions, suggesting systematic confusion rather than uncertainty.
...
We presented VANS, a transformer-based architecture for solving Raven's Progressive Matrices through context-based reasoning. Our approach combines frozen DINOv2-L visual features with a lightweight reasoning module, achieving 79.5\% accuracy on four I-RAVEN configurations with training on additional configurations ongoing.

\subsection{Objectives Assessment}

Our project objectives have been partially met with promising results. For architecture design, we designed and implemented a transformer-based architecture that effectively encodes context and reasons about candidates, achieving 79.5\% accuracy on four configurations with center\_single reaching 97.1\%. For feature representation, we demonstrated that frozen DINOv2 features provide effective visual representations for RPM solving, enabling strong performance without task-specific visual training, confirming the transferability of foundation model features to visual reasoning tasks. For performance evaluation, we systematically evaluated performance across four I-RAVEN configurations, identifying strengths such as excellent performance on simple single-object patterns (97.1\%) and areas for improvement on distributed grid configurations (68-75\%).
...
In summary, VANS demonstrates that context-based reasoning with foundation model features can effectively solve Raven's Progressive Matrices, achieving 79.5\% accuracy on four configurations with particularly strong performance on single-object patterns (97.1\%). This provides a strong baseline for future work in visual analogical reasoning, with ongoing training expected to further improve results across all seven configurations.
